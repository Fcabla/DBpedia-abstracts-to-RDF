...
class Triple:
    ...
    def set_lexicalizations(self, lex_dict):
    ...
    for triple in triples:
        ...
        for key in term_URI_dict.keys():
            if key in subj.lower():
                ...
                keys.append(key)
                # debugging ubj
                #lex_dict[key] = term_URI_dict[key]
        ...
        if len(keys) > 1:
            ...
            # debugging subj
            lex_dict[list(term_URI_dict.keys())[list(term_URI_dict.values()).index(s_candidates[0])]] = s_candidates[0]

        if verb == "be" and prep == DEFAULT_VERB:
            # the case of the verb to be has to be treated different from the rest
            objct, surf_objct = get_dbo_class(objct, cla_lex_table)
            o_candidates = [objct]
            pred = prop_lex_table[verb][prep]

            # debugging verb and objct
            lex_dict[verb+" "+prep] = pred
            o_lex_dict[surf_objct] = objct
            ...
            for key in term_URI_dict.keys():
                if key in objct.lower():
                    if key != "the":
                        ...
                        # debugging objct
                        o_lex_dict[key] = term_URI_dict[key]

            # Lexicalization predicate     
            # debugging: verb+" "+prep always vs real surface
        ...
        # Build triple
        #new_triple.append(lex_dict)
        for s in s_candidates:
            for o in o_candidates:
                temp_dict = lex_dict.copy()
                temp_dict[list(o_lex_dict.keys())[list(o_lex_dict.values()).index(o)]] = o
                if isinstance(pred,list):
                    # temporal fix, to be changed
                    # pred = pred.pop()
                    pred = get_best_candidate(s, o, pred, term_types_dict, dbo_graph)
                    temp_dict[verb+" "+prep] = pred
                else:
                ...
                new_triple.set_lexicalizations(temp_dict)
                new_triples.append(new_triple)
...
def get_dbo_class(objct, cla_lex_table):
    ...
    if objct in cla_lex_table.keys():
        return URIRef(cla_lex_table[objct]), objct
    else:
        candidates = []
        ...
        if candidates:
            ...
            return URIRef(result), key
        else:
            return Literal(objct), objct
...
def main():
    ...
    benchmark_df = pd.DataFrame(columns=['sent', 'text_triple', 'rdf_triple', 'lexicalizations', 'label'])
